...
import {
  ...
  Layers, Users, Building2, Target, Clock,
} from 'lucide-react';
import { WorkspaceDetail, WorkspaceData, WorkspaceMember } from './WorkspaceDetail';
...
function KpiCard({ kpi, i }: { kpi: typeof KPIS[0]; i: number }) {
  return (
    <motion.div
      ...
      whileHover={{ y: -4, borderColor: `${kpi.color}40` } as any}
      style={{
...
function WorkspaceCard({ ws, i, onOpen }: { ws: WorkspaceData; i: number; onOpen: () => void }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4, borderColor: `${color}35` } as any}
      style={{
        background: C.card, backdropFilter: 'blur(16px)',
        border: `1px solid ${C.border}`, borderRadius: '16px',
    ...
    >
      ...
      <motion.button
        whileHover={{ backgroundColor: `${color}25`, borderColor: `${color}60` } as any}
        whileTap={{ scale: 0.97 }}
        onClick={onOpen}
        style={{
          ...
          {workspaces.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
              ...
              <motion.div
                ...
                whileHover={{ borderColor: 'rgba(0,194,168,0.4)' } as any}
                onClick={onCreateWorkspace}
                style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {INSIGHTS.map((item, i) => (
              <motion.div
                ...
                whileHover={{ borderColor: `${item.color}35` } as any}
                style={{
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
            {QUICK_ACTIONS.map((action, i) => (
              <motion.button
                ...
                transition={{ delay: 0.1 + i * 0.07, duration: 0.4 }}
                whileHover={{ y: -3, borderColor: `${action.color}50`, backgroundColor: `${action.color}10` } as any}
                whileTap={{ scale: 0.97 }}
                style={{
                  display: 'flex', alignItems: 'center', gap: '10px',
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100vw', background: C.bg, overflow: 'hidden', fontFamily: 'DM Sans, sans-serif', color: C.text }}>
      ...
      <aside style={{
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <motion.button
                ...
                whileHover={{ backgroundColor: isActive ? 'rgba(0,194,168,0.14)' : 'rgba(255,255,255,0.04)' } as any}
                style={{
                  display: 'flex', alignItems: 'center', gap: '10px',
        ...
        <div style={{ padding: '0 12px' }}>
          <motion.button
            whileHover={{ backgroundColor: 'rgba(239,68,68,0.08)', color: '#EF4444', borderColor: 'rgba(239,68,68,0.2)' } as any}
            onClick={onLogout}
            style={{
              display: 'flex', alignItems: 'center', gap: '10px',
              padding: '10px 12px', borderRadius: '10px',
